...
// الحصول على الفلتر من الرابط
$filter = $_GET['filter'] ?? 'all';

// اختيار الاستعلام حسب الفلتر المطلوب
if ($filter === 'completed') {
    $stmt = $conn->prepare("
        SELECT * FROM tasks 
    ...
    // الترتيب: غير المكتملة أولًا، ثم حسب الأولوية، ثم الأحدث
    $stmt = $conn->prepare("
        SELECT * FROM tasks 
        WHERE user_id = ? 
        ORDER BY 
      ...
      <ul class="list-group list-group-flush">
        <?php while ($row = $result->fetch_assoc()): ?>
          <li class="list-group-item">
            ...
            <!-- رأس المهمة: اسم المهمة على جهة، والفئة + الأهمية على الجهة المقابلة -->
            <!-- رأس المهمة: الاسم + الأهمية + الفئة -->
...
<p class="task-date <?= !empty($row['completed']) ? 'text-decoration-line-through text-muted' : '' ?>">
  Due: <?= htmlspecialchars($row['due_date']) ?>
            ...
            <!-- أزرار الحذف والتكميل في الأسفل على اليمين -->
